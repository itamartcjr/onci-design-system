...
const usageByArea = (page: SystemPageKey) => {
  if (page.startsWith('foundation-')) return {
    ...
    responsive: 'A fundação deve declarar o que permanece e o que muda entre Desktop, Tablet e Mobile. Responsividade é comportamento, não miniaturização.',
  ...
  if (page.startsWith('component-')) return {
    ...
    responsive: 'O componente pode mudar de largura, distribuição, densidade ou interação no mobile sem perder função e acessibilidade.',
    accessibility: 'Estados de foco, teclado, disabled, loading, mensagens e alvo de toque devem continuar perceptíveis e operáveis.',
  };
  if (page.startsWith('pattern-')) return {
    ...
    avoid: 'Não trate o pattern como screenshot rígido. Conteúdo, quantidade de itens e breakpoint podem mudar sem quebrar sua estrutura.',
    responsive: 'Documente explicitamente a reorganização entre Desktop, Tablet e Mobile, inclusive quando a interação muda de natureza.',
    accessibility: 'A ordem visual deve continuar coerente com DOM, foco, leitura assistiva e mensagens de estado.',
  };
  if (page.startsWith('tokens-')) return {
    ...
    avoid: 'Não crie token para cada valor isolado. Um token precisa representar uma decisão reutilizável ou uma relação estável.',
    ...
    accessibility: 'Tokens semânticos de foco, texto, feedback e motion reduzido ajudam a tornar acessibilidade consistente entre componentes.',
  };
  return {
    ...
    avoid: 'Não copie a composição como template universal quando o contexto de conteúdo ou produto pedir outra solução.',
    responsive: 'A expressão visual pode mudar por frente e viewport, preservando hierarquia, contraste e função.',
